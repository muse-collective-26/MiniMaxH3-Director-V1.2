...
- Total Duration is editable again after briefly being computed-only during this update's development — it's the one source of truth for chunk count (`⌈Total Duration ÷ Chunk Size⌉` chunks), and shrinking it now actually removes the trailing chunk(s) instead of leaving them stuck and invisible.
